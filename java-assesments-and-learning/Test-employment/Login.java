...
public class Login implements ActionListener{
	...
	private void buildPanel(){
		...
		layout.putConstraint(SpringLayout.NORTH, usernameLabel, 130, SpringLayout.NORTH, container);
		...
		layout.putConstraint(SpringLayout.NORTH, usernameTextField, 130, SpringLayout.NORTH, container);
	...
	public void actionPerformed(ActionEvent event){
		if(event.getSource() == loginButton){
			...
			String query = "SELECT username,password FROM staff WHERE username = '" + username + "' AND password = '" + new String(password) + "'";
			...
			try {
				if(!resultSet.next()){
					...
					new MainMenu(frame, container, panel, database);
				}
